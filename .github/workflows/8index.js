...
if(steel == 'XAUUSD'){event_horizon = 0.40; stop_loss = 0.10}
else if(steel == 'USDJPY'){event_horizon = 0.035; stop_loss = 0.035}
else if(steel == 'ETHUSD'){event_horizon = 0.30; stop_loss = 0.10}
else {event_horizon = 0.00030; stop_loss = 0.00001}
...
let what_was;
  while(new Date(new Date().getTime() + 10800000) < black_hole){
    ...
    const e = await page.evaluate((what_was) =>{
      let what_is;
      //if(what_was == undefined){ 
         what_is = document.querySelectorAll('div.svelte-7c7doc')[0]; //}
      //else if(what_was == 'buy'){what_is = document.querySelectorAll('div.svelte-7c7doc.blue')[0]; }
      //else if(what_was == 'sell'){ what_is = document.querySelectorAll('div.svelte-7c7doc.red')[0];}
      if (what_is !== undefined) {
        const price = document.querySelectorAll('div.td.svelte-1gisemh.right')[4].innerText;
        //return { whatIs: what_is, price: price };
        return { muuu: what_is.innerText, aaaa: price //, oooo: what_was
               };
      }
    }, what_was)
console.log(e)
    ...
    if(e !== undefined){
      // what_was = e.oooo;
       const targetPage = page;
       await scrollIntoViewIfNeeded([["body > div.layout.svelte-ezbv2f > div.bot-panel.svelte-1l3uzb9 > div > div.wrapper.svelte-14lspcp > div > div > div:nth-child(2) > div:nth-child(11)"], ["xpath//html/body/div[2]/div[4]/div/div[2]/div/div/div[2]/div[11]"]], targetPage, timeout);
       let element = await waitForSelectors([["body > div.layout.svelte-ezbv2f > div.bot-panel.svelte-1l3uzb9 > div > div.wrapper.svelte-14lspcp > div > div > div:nth-child(2) > div:nth-child(11)"], ["xpath//html/body/div[2]/div[4]/div/div[2]/div/div/div[2]/div[11]"]], targetPage, {
        timeout,
        visible: true
    });
       await element.click({ offset: { x: 9.046875, y: 13,},});
       let finish;
       let initial = parseFloat(e.aaaa.replace(/ /g, ''));
       while(new Date(new Date().getTime() + 10800000) < black_hole){  
          const d = await page.evaluate(() =>{
          let price = document.querySelectorAll('div.td.svelte-1gisemh.right')[4].innerText;
          price = parseFloat(price.replace(/ /g, ''));
          return price 
          });
          console.log(d)
          ...
          if(e.muuu == 'buy'){ 
            if(d > initial){ initial = d; }   
            console.log(initial, initial - d)
            if(initial - d >= stop_loss){console.log('buy')
               await page.evaluate(() => {document.querySelectorAll('button.close.svelte-7c7doc.active')[0].click()}); finish = 1; console.log(8)
            }
          }
          ...
          if(e.muuu == 'sell'){ 
            ...
            if(d - initial <= stop_loss){console.log('sell')
               await page.evaluate(() => {document.querySelectorAll('button.close.svelte-7c7doc.active')[0].click()}); finish = 1;
            } 
          }
...
//const f = new Date(new Date().getTime() + 10800000+5000);
//while(new Date(new Date().getTime() + 10800000) < f){}
 //   console.log(9)
