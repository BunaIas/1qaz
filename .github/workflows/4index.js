...
let singularity = '2023-08-17 | 15:30 | 12:01 | 3  12:00 x ETH x Industrial Production (MoM) (Jun) x 479593';
...
const timeout = 6000;
...

   {
    ...
    if (inputType === 'select-one') {
        await changeSelectElement(element, "1059116110")
    } else if (["textarea", "text", "url", "tel", "search", "password", "number", "email"].includes(inputType)) {
        await typeIntoElement(element, "1059116110");
    } else {
        await changeElementValue(element, "1059116110");
    }
}
    ...
    
{
    ...
    const inputType = await element.evaluate(el=>el.type);
    if (inputType === 'select-one') {
        await changeSelectElement(element, "1NESEQDEB3")
    } else if (["textarea", "text", "url", "tel", "search", "password", "number", "email"].includes(inputType)) {
        await typeIntoElement(element, "1NESEQDEB3");
    } else {
        await changeElementValue(element, "1NESEQDEB3");
    }
}
